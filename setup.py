#!/usr/bin/env python3
"""
Run 'python3 setup.py install' to install Nanopore barcode binner.
"""
...
exec(open('nanopore_barcode_binner/version.py').read())

with open('README.md', 'rb') as readme:
    LONG_DESCRIPTION = readme.read().decode()


class NanoporeBarcodeBinnerBuild(build):
    """
    ...
    def run(self):
        ...
        self.execute(compile_cpp, [], 'Compiling Nanopore barcode binner: ' + ' '.join(make_cmd))


class NanoporeBarcodeBinnerInstall(install):
    """
    ...
    def run(self):
        ...
        shutil.copyfile(os.path.join('nanopore_barcode_binner', 'cpp_functions.so'),
                        os.path.join(self.install_lib, 'nanopore_barcode_binner', 'cpp_functions.so'))


class NanoporeBarcodeBinnerClean(Command):
    """
...
setup(name='nanopore_barcode_binner',
      version=__version__,
      description='Nanopore barcode binner',
      long_description=LONG_DESCRIPTION,
      url='http://github.com/rrwick/nanopore_barcode_binner',
      author='Ryan Wick',
      author_email='rrwick@gmail.com',
      license='GPL',
      packages=['nanopore_barcode_binner'],
      entry_points={"console_scripts": ['nanopore_barcode_binner = nanopore_barcode_binner.nanopore_barcode_binner:main']},
      zip_safe=False,
      cmdclass={'build': NanoporeBarcodeBinnerBuild,
                'install': NanoporeBarcodeBinnerInstall,
                'clean': NanoporeBarcodeBinnerClean}
      )
